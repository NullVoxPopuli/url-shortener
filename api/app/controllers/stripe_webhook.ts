...
export default class StripeWebhookController {
  async handle({ request, response }: HttpContext) {
    ...
    if (typeof eventId !== 'string') {
      response.status(400);
      return response.json({ error: 'Missing event id' });
    }

    const customerId = (event.data.object as { customer?: unknown })?.customer;
    const normalizedCustomerId = typeof customerId === 'string' ? customerId : null;

    const isFirstDelivery = await markStripeEventProcessed({
      eventId,
